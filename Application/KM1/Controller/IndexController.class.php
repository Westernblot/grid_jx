...
class IndexController extends Controller {
    ...
    public function index($apply_detail_type = '') {
        ...
        $question = M('Question_k1');

        //=======================随机查询出100道题目========================================

        $quesA1 = $question -> field('id,theme,qid') -> where("question_type_code='A1' ") -> order("Rand()") -> limit(50) -> select();
        $quesA2 = $question -> field('id,theme,qid') -> where("question_type_code='K1' ") -> order("Rand()") -> limit(50) -> select();
    ...
    public function exportTheme($qid = 0) {
        ...
        foreach ($theme_option as $key => $value) {
            $one_option = $value['option````_answer'] . "、" . $value['content'] . "<br>";
            $id = $value['id'];
