...
class IndexController extends Controller {
	...
	public function index($apply_detail_type = '') {
		...
		$question = M('Question');
		if ($apply_detail_type == '经营性道路货物运输驾驶员' || $apply_detail_type == '经营性道路旅客运输驾驶员') {

			//=======================随机查询出100道题目========================================

			$quesX1 = $question -> field('id,theme') -> where("question_type_code='X1' and apply_detail_type = '{$apply_detail_type}'") -> order("Rand()") -> limit(20) -> select();
			$quesA1 = $question -> field('id,theme') -> where("question_type_code='A1' and apply_detail_type = '{$apply_detail_type}'") -> order("Rand()") -> limit(40) -> select();
			$quesA2 = $question -> field('id,theme') -> where("question_type_code='A2' and apply_detail_type = '{$apply_detail_type}'") -> order("Rand()") -> limit(40) -> select();
			...
			$quesA1 = $question -> field('id,theme') -> where("question_type_code='A1' and apply_detail_type = '{$apply_detail_type}'") -> order("Rand()") -> limit(50) -> select();
			$quesA2 = $question -> field('id,theme') -> where("question_type_code='A2' and apply_detail_type = '{$apply_detail_type}'") -> order("Rand()") -> limit(50) -> select();
	...
	public function exportTheme($qid = 0) {
		...
		foreach ($theme_option as $key => $value) {
			...
			if (strlen($resQuestion['rightanswer']) == 1) {//单选、多项提示
				$html_option = $html_option . "<input type='radio' name='chk_option' value='{$value['option_answer']}' />" . $one_option;

			} else {

				$html_option = $html_option . "<input type='checkbox' name='chk_option' value='{$value['option_answer']}' />" . $one_option;
			}
